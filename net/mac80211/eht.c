...
ieee80211_send_eml_op_mode_notif(struct ieee80211_sub_if_data *sdata,
				 ...
				 int ctrl_field_len)
{
	/* ieee80211_mgmt header + category + action_code + dialog_token */
	int len = offsetof(struct ieee80211_mgmt, u.action.u.eml_omn) + 2;
	struct ieee80211_local *local = sdata->local;
	struct ieee80211_mgmt *mgmt;
	struct sk_buff *skb;

	len += ctrl_field_len; /* control field len */
	...
	/* Copy control field from the notification frame received from the
	 * client.
	 */
	memcpy(&mgmt->u.action.u.eml_omn.control,
	       &req->u.action.u.eml_omn.control, ctrl_field_len);
				    ...
				    struct sk_buff *skb)
{
	...
	if (control & IEEE80211_EML_CTRL_EMLSR_MODE) {
		...
		ctrl_field_len += sizeof(__le16); /* eMLSR link_bitmap */
		...
		if (skb->len < len + ctrl_field_len + emlsr_param_update_len)
			return;
	...
	if (control & IEEE80211_EML_CTRL_EMLMR_MODE) {
		...
		ctrl_field_len += sizeof(__le16); /* eMLMR link_bitmap */
		ctrl_field_len++; /* eMLMR mcs_map_count */
		if (skb->len < len + ctrl_field_len)
			return;
	...
	ieee80211_send_eml_op_mode_notif(sdata, mgmt, ctrl_field_len);
}
